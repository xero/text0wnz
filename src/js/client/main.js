...
const initializeAppComponents = async () => {
	...
	Toolbar.addLazy($('shading-brush'), async () => {
		const { createShadingController, createShadingPanel } = await import('./freehand_tools.js');
	...
	Toolbar.addLazy($('character-brush'), async () => {
		const { createShadingController, createCharacterBrushPanel } = await import('./freehand_tools.js');
	...
	Toolbar.addLazy($('attrib'), async () => {
		const { createAttributeBrushController } = await import('./freehand_tools.js');
	...
	State.modal.focusEvents(
		...
		() => {
			...
			if (characterBrush && characterBrush.unignore) {characterBrush.unignore();}
		},
	);
