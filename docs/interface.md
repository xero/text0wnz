...
- **Binary Text** (.bin) - Raw binary format
- **XBin** (.xb) - Extended Binary format with embedded fonts and palettes
- **PNG** - Export as image
- **UTF-8 / Plain Text** - Export as text
...
- Ideal for detailed artwork and shading
...
- **Modern** - Contemporary XBIN fonts (Topaz, Bloody, etc.)
- **Commodore 64** - PETSCII fonts
- **Amiga** - MicroKnight, Topaz variants
- **Specialized** - Arabic, Cyrillic, Greek, Hebrew, and more

### iCE Colors

Extended 16-color background palette (normally only 8 are available in ANSI). Required for certain artwork styles.

**Note:** iCE colors use the blink bit for additional background colors, disabling blinking text.
