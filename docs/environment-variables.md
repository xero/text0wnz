...
**Note:** Must end with `/` or it will be added automatically. Leading slash is removed.
...
**Note:** Currently not used in vite.config.js but may be referenced in client code.
...
PORT=8080 bun server

# Docker
docker run -e PORT=8080 text0wnz:latest
```
...
```bash
# Docker
docker run -e SESSION_KEY=your-secret-key text0wnz:latest
```

**Security:**
- Should be a long random string in production
- Used for session cookie signing
- Auto-generated in Docker if not provided
...
**Solution:** Set `SESSION_KEY` to a secure random value
