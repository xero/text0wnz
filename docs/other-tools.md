...
#!/bin/bash
# Run linting
bun lint:check || exit 1

# Run formatting check
bun format:check || exit 1
...
For scaling collaboration server with multiple instances.
...
**Integration:** Replace file-based session storage with Redis for horizontal scaling.
